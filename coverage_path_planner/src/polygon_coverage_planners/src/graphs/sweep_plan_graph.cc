...
bool SweepPlanGraph::create() {
  ...
  for (size_t cluster = 0; cluster < polygon_clusters_.size(); ++cluster) {
    ...
    } else if(settings_.sweep_preset_direction){
      const Vector_2 preset_vec(cos(settings_.sweep_angle),sin(settings_.sweep_angle));
      Direction_2 preset_dir(preset_vec);
      visibility_graph::VisibilityGraph vis_graph(polygon_clusters_[cluster]);
      cluster_sweeps.resize(1);
      ROS_ASSERT(settings_.sensor_model);
      if (!computeSweep(polygon_clusters_[cluster], vis_graph,
                        settings_.sensor_model->getSweepDistance(), preset_dir,
                        true, &cluster_sweeps.front())) {
        ROS_ERROR_STREAM(
            "Cannot compute preset sweep for cluster: " << cluster);
        return false;
      }     
    } else {
      ROS_ASSERT(settings_.sensor_model);
...
bool SweepPlanGraph::computeDecomposition() {
  ...
  if(!settings_.sweep_preset_direction){
    ...
    // TODO: Implement judgment on successness
    const Vector_2 preset_vec_vertical(cos(settings_.sweep_angle+mDeg2rad(90)),sin(settings_.sweep_angle+mDeg2rad(90)));
    Direction_2 preset_dir_vertical(preset_vec_vertical);
    polygon_clusters_ = computeBCD(settings_.polygon,preset_dir_vertical);
    ROS_INFO_STREAM("Successfully created pre-set direction decomposition with "
                          << polygon_clusters_.size() << " polygon(s).");
  }
  timer_decom.Stop();
