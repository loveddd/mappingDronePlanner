...
class CoveragePlanner : public PolygonPlannerBase {
 public:
  // Constructor
  CoveragePlanner(const hecto::mission::common::GridOptions* request)
      : PolygonPlannerBase(request),
    ...
    // Test
    // Get sweep angle
    double sweep_angle_temp = mDeg2rad(45);
    if(request->sweepwithpresetdirection()){
      sweep_preset_direction_ = true;
      sweep_angle_temp = mDeg2rad(request->rotationdeg());
      sweep_angle_ = std::make_optional(sweep_angle_temp);
      ROS_INFO_STREAM("Sweep pre-set direction: " << sweep_preset_direction_ << "\n" << "Sweep angle(deg): " << request->rotationdeg());
    }
    ...
    // TODO: change to entry point later
    double scalar = (40075*cos(mDeg2rad(request->polygon()[0].latitudedeg()))/360*abs(cos(sweep_angle_temp))+111.32*abs(sin(sweep_angle_temp)))*1000;
    
    double lateral_footprint_temp = 10;
    if (lateral_footprint_temp !=  request->lanedistancem()) {
      lateral_footprint_temp =  request->lanedistancem();
    }
    lateral_footprint_ = std::make_optional(lateral_footprint_temp/scalar);

    double lateral_overlap_temp = 0;
    if (true) {
      lateral_overlap_ = std::make_optional(lateral_overlap_temp);
    }

    double lateral_fov_temp = -2;
    if (true) {
  ...
  std::vector<Polygon_2> getDecomposition(){
    return planner_->getDecomposition();
  }
 ...
 private:
  ...
  inline bool resetPlanner() override {
    ...
    settings.sweep_preset_direction = sweep_preset_direction_;
    if(sweep_angle_)   settings.sweep_angle = *sweep_angle_;
  ...
  bool sweep_preset_direction_;
  std::optional<double> sweep_angle_;
