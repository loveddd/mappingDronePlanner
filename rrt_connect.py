...
class RrtConnect:
    def __init__(self, s_start, s_goal, step_len, goal_sample_rate, iter_max):
        ...
        self.env = env.Env([s_start,s_goal])
        self.plotting = plotting.Plotting(s_start, s_goal)
        self.utils = utils.Utils([s_start,s_goal])
...
def main():
    ...
    step = max(abs(x_start[0]- x_goal[0]),abs(x_start[1]- x_goal[1]))/2.0

    rrt_conn = RrtConnect(x_start, x_goal, step, 0.05, 5000)
